...
def get_pose3D(video_path, output_dir):
    ...
    args.layers, args.channel, args.d_hid, args.frames = 3, 512, 1024, 27
    ...
    for i in tqdm(range(video_length)):
        ...
        ## input frames
        if use_hist_only:
            start = max(0, i - 2*args.pad)
            end =  min(i, len(keypoints[0])-1)
        else:
        ...
        input_2D_no = np.tile(input_2D_no, (27, 1, 1))
        ...
        print(f"input_2D_no.shape: {input_2D_no.shape}")
        ...
        print(f"left_pad, right_pad: {left_pad}, {right_pad}")
        print(f"input_2D_no.shape: {input_2D_no.shape}")
